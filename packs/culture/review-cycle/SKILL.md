...
4. **Peer feedback relay, if routed through this skill:** relay each response to the reviewing manager **verbatim and individually**, attributed exactly per the cycle's `peer_feedback_attribution` setting. Never combine two people's words into one sentence, never write "most peers said X," never add a qualifier the peer didn't use, never attach an interpretation. If `peer_feedback_attribution` is unset for this cycle, treat every piece as identifying and escalate to the accountable human to decide before relaying anything.
