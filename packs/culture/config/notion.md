...
**Gap to flag for the integrator:** `platform/contracts/notion-employees.md` and its index row in `platform/contracts/README.md` currently list only `onboarder`/`people-ops` as consumers. This pack is a third reader and should be added to both once the contract file is next touched — noted here rather than edited directly, since `platform/` is outside this build's ownership.
