...
Every draft an agent produces resolves one of four ways (`sent_unedited`, `sent_light_edit`, `sent_rewrite`, `discarded`), and that resolution is evidence. But the *record* of one specific resolution — this draft, this candidate, this diff — is linkable to a real person. Under DPDP (master PRD §6.8.6) that record cannot live in git: git is durable, replicated, cloned to laptops, and never truly deleted. So the platform keeps two tiers, deliberately apart:
